			...
			if (me.getChannel().getName().equalsIgnoreCase("rezxis-server-operation")) {
				...
				if (msg.startsWith("/eval ")) {
					...
					if (msg.startsWith("sync ")) {
						...
						DiscordBot.client.send(gson.toJson(new SyncExecuteScriptPacket(url,script,ScriptTarget.Host,id)));
					} else if (msg.startsWith("game ")) {
						script = msg.replace("game ", "");
						int id = Integer.valueOf(script.split(" ")[0]);
						script = script.replace(id+" ", "");
						DiscordBot.client.send(gson.toJson(new SyncExecuteScriptPacket(url,script,ScriptTarget.Game,id)));
					} else if (msg.startsWith("bungeecord ")) {
						script = msg.replace("bungeecord ", "");
						DiscordBot.client.send(gson.toJson(new SyncExecuteScriptPacket(url,script,ScriptTarget.Bungee,-1)));
					} else if (msg.startsWith("lobby")) {
						script = msg.replace("lobby ", "");
						DiscordBot.client.send(gson.toJson(new SyncExecuteScriptPacket(url,script,ScriptTarget.Lobby,-1)));
					}
				}
			}
		} else if (event instanceof ReadyEvent) {
			new Thread(new JDAGameTask()).start();
